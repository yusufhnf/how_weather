# Guidelines for Gemini CLI - How Weather Project
...
* **Adding Dev Dependencies:** To add a development dependency, use the `pub` tool, if it is available, with `dev:<package name>`. Otherwise, run `flutter
...
* **Component Themes:** Use specific theme properties (e.g., `appBarTheme`,
  `elevatedButtonTheme`, `cardTheme`) to customize the appearance of individual Material
  components.
...
* **No useless documentation:** If the documentation only restates the obvious
  from the code's context, it's not helpful. Good documentation provides context
